...
int main() {
    const string path = "../sample/hello.o";
    auto elf_file = make_unique<File>(path);

    auto file_header = elf_file->get_header();
    auto id = file_header->get_id();
    cout << "File: " << path.c_str() << endl;
    cout << "File Type: " << Format::get_file_type(file_header->get_type()) << endl;
