...
File::File(const string &filename) {
    ...
    header = make_unique<Header>(istream);
    ...
    if (sh_count > 0) {
        ...
        for (int i = 0; i < sh_count; i++) {
            auto header = make_unique<SectionHeader>(istream, sh_pos);
        ...
        for (size_t i = 0; i < section_headers.size(); i++) {
            ...
            if (sh->get_type() == 3) {
                section = make_unique<StringsSection>(istream, sh->get_section_offset(), sh->get_size());
                ...
                section = make_unique<Section>(istream, sh->get_section_offset(), sh->get_size());
